 ...
 * static_cache_html_mw_disable_cache() function. NOTE: this function only prevents the
...
if (!defined('OFFSET_PATH')) {
	...
	require_once(dirname(dirname(__FILE__)).'/zp-core/admin-globals.php');
}

$plugin_is_filter = 9|ADMIN_PLUGIN|THEME_PLUGIN;
$plugin_description = gettext("Adds static HTML cache functionality to Zenphoto.");
$plugin_author = "Marcus Wong (original code by Malte Müller (acrylian), Stephen Billard (sbillard))";
$plugin_version = '1.4.1';

$option_interface = 'staticCacheMW_options';

require_once(dirname(dirname(__FILE__)).'/zp-core/functions.php');

zp_register_filter('admin_utilities_buttons', 'static_cache_html_mw_purgebutton');
...
if (OFFSET_PATH) {
	if (isset($_GET['action']) && $_GET['action']=='clear_html_cache' && zp_loggedin(ADMIN_RIGHTS)) {
		...
		$_zp_HTML_cache = new staticCacheMW();
 ...
 *
 */
class staticCacheMW_options {

	function staticCacheMW_options() {
 ...
 * @return array
 */
function static_cache_html_mw_purgebutton($buttons) {
	$buttons[] = array(
								'enable'=>true,
 ...
 */
function static_cache_html_mw_disable_cache() {
